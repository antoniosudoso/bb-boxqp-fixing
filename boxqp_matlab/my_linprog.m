function [x,fval,exitflag,output,lambda] = my_linprog(f,A,b,Aeq,beq,lb,ub,x0,options)
...
if isfield(options,'Method')
    if strcmp(options.Method, 'dual simplex')
        params.Method = 1;
    elseif strcmp(options.Method, 'barrier')
        params.Method = 2;
    else
        options.Method = 3; % concurrent
    end
end

if isfield(options,'Threads')
